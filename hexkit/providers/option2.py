...
class AsyncCM(ABC):
    def __init__(self):
        """Initialize bool flag indicating whether the instance is currently
        'in context' (typically inside a `with` block) or not."""
        self._in_context = False

    async def __aenter__(self):
        """Run the setup function and update the flag."""
        self._in_context = True
        self.setup()

    async def __aexit__(self, exc_type, exc_value, exc_trace):
        """Run the teardown function and update the flag."""
        self._in_context = False
        self.teardown()

    @abstractmethod
    async def setup(self) -> None:
        """Custom logic to setup an instance."""
        ...

    @abstractmethod
    async def teardown(self) -> None:
        """Custom logic to teardown an instance"""
        ...
    ...
    def ensure_context(method):
        ...
        async def guarded_method(self: AsyncCM, *args, **kwargs):
            """Wraps method to ensure that it is executed in context."""
            if not self._in_context:  # pylint: disable=protected-access
...
class KafkaEventPublisher(AsyncCM, EventPublisherProtocol):

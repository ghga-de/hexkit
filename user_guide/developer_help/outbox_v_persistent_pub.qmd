---
title: "Outbox vs Persistent Publisher: Choosing the Right One"
---
...
That command delegates to an asynchronous entrypoint function which constructs the
config and the publisher. See the
[persistent publisher usage example](../providers/mongokafka.qmd#usage-example-in-service-abc)
for how `get_persistent_publisher` is put together:

```python
# In a module housing asynchronous entrypoint functions:
from my_service.config import Config

# Import the function that returns a configured PersistentKafkaPublisher or
# MongoKafkaDaoPublisher instance.

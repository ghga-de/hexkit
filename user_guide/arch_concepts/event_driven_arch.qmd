...
To simplify Kafka usage in services, hexkit features protocols and providers. Rather than implementing this functionality from scratch in every new project, hexkit provides the `KafkaEventPublisher` and `KafkaEventSubscriber` provider classes, along with a `KafkaConfig` Pydantic configuration class.
